...
module.exports = function (grunt) {
    ...
    var email     = grunt.option('email')  || 'wuliwux1@gmail.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new onewire name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/onewire.png')) {
            fs.renameSync(__dirname + '/admin/onewire.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/onewire.html')) {
            fs.renameSync(__dirname + '/widgets/onewire.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/onewire/js/onewire.js')) {
            fs.renameSync(__dirname + '/widgets/onewire/js/onewire.js', __dirname + '/widgets/onewire/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/onewire')) {
            fs.renameSync(__dirname + '/widgets/onewire',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Onewire/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Onewire'
                        },
                        {
                            match: /wuliwux/g,
                            replacement: author
